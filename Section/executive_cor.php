...
<head>
    ...
    <style>
        body {
            ...
            padding: 0;
            font-size: 12pt;
            background: linear-gradient(358deg, #1a6511, transparent);
        }
...
$executive = $_GET['executive'] ?? false;
...
CONCAT(yl.Year_level) AS `yrlevel`,
enstd.Semester,
stdtype.type,
CONCAT(
    pl.Program_code,
...
enstd.Enrolled_student_id as `es_std_id`
FROM
...
?>
        <span style="position: absolute;" class="pgs">Page <?php echo $pagecnt; ?></span>
        ...
        <div class="main-page <?php echo ($pagecnt < $nr ? 'section-break' : ''); ?>">
            ...
            <div class="watermarkdata" contenteditable="true">
                <p>PATEROS TECHNOLOGICAL COLLEGE</p>
                <p>ENROLLED</p>
                <p><u>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo strtoupper($semsy[1]); ?> &nbsp;&nbsp;</u>TRIMESTER S.Y: <u><?php echo $semsy[0]; ?></u></p>
                <p>SIGNATURE:____________</p>
            ...
            <div class="onprintdiv" style="margin-bottom:-50px;">
                <img src="../assets/img/ptclogo.png" alt="" height="65px;" class="imglugu" style="float:left; margin-left: 255px;">
            ...
            <table style="width: 100%;font-size:14px;">
                <tbody>
                    <tr>
                        ...
                        <td>SEM:<u><?php echo strtoupper($row['Semester']) ?> <?php echo 'TRIMESTER' ?></u></td>
                        <td>STATUS:<u><?php echo 'EXECUTIVE'  ?></u></td>
                    </tr>
                    <tr>
            ...
            $sql2 = "SELECT
                ...
                CONCAT(pl.Program_code, ' - ', yl.Year_level, ' - ', sec.letter) AS `Section`,
                GROUP_CONCAT(DISTINCT cs.Day ORDER BY cs.Day) AS `day`,
                GROUP_CONCAT(DISTINCT CONCAT(cs.Starting_time, ' - ', cs.Ending_time) ORDER BY cs.Starting_time) AS `Time`,
                CONCAT('Prof. ', prof.Instructor_lastname, ', ', prof.Instructor_firstname) AS `Prof.Name`,
                IF(rr.Room_no = 'Online', '', rr.Room_no) AS `room`,
                IF(s.complab IS NULL OR s.complab = 0, '', s.complab) AS complabs
            FROM
                `enrolled_subjects` esub
            LEFT JOIN `enrolledstudents` es 
                ON es.Enrolled_student_id = esub.Enrolled_student_id
            LEFT JOIN `curriculums` c 
                ON esub.Curriculum_id = c.Curriculum_id
            LEFT JOIN `class_schedules` cs 
                ON c.Curriculum_id = cs.Curriculum_id
                AND es.Program_section = cs.Prog_year_section_id
            LEFT JOIN `program_year_sections` pys 
                ON cs.Prog_year_section_id = pys.Prog_year_section_id
            LEFT JOIN `program_year` py 
                ON py.Program_year_id = pys.Program_year_id
            LEFT JOIN `program_list` pl 
                ON py.Program_list_id = pl.Program_list_id
            LEFT JOIN `year_level` yl 
                ON py.Year_level_id = yl.year_level_id
            LEFT JOIN `sections` sec 
                ON sec.Section_id = pys.Section_id
            LEFT JOIN `subjects` s 
                ON c.Subject_id = s.Subject_id
            LEFT JOIN `instructors` prof 
                ON cs.Instructor_id = prof.Instructor_id
            LEFT JOIN `rooms` rr 
                ON cs.Room_id = rr.Room_id
            WHERE  es.Enrolled_student_id = '" . $row['es_std_id'] . "'
                ...
                <table border="1" style="border-collapse: collapse;width: 100%;font-size:13px;">
                    ...
                    <tbody>
                        ...
                        ?> <tr>
                                ...
                                <td style="text-align:center;"><?php echo $r[8] ?></td>
                    ...
                    <table style="font-size:13px;">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>₱<?php echo ($executive ? $miscdata[7] : $miscdata[6]) ?></td>
                            </tr>
                            <tr>
                                <td>Student Handbook:</td>
                                <td>₱<?php echo ($executive ? $miscdata[6] : $miscdata[7])  ?></td>
                            </tr>
                            <tr>
                                <td>Library Fee:</td>
